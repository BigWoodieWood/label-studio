...
def handle_task_created(sender, instance, created, **kwargs):
    """
    Signal handler to automatically create initial state when a task is created.
    ...
    if created:
        try:
            ...
            StateManager.transition_state(
                ...
                reason='Task created automatically',
            )
            logger.info(f'Created initial FSM state for Task {instance.pk}')
        except Exception as e:
            logger.error(f'Failed to create initial FSM state for Task {instance.pk}: {e}')


def handle_annotation_created(sender, instance, created, **kwargs):
    """
    Signal handler to automatically create initial state when an annotation is created.
    ...
    if created:
        try:
            ...
            StateManager.transition_state(
                entity=instance,
                new_state='DRAFT',
                transition_name='create_annotation',
                reason='Annotation created automatically',
            )
            logger.info(f'Created initial FSM state for Annotation {instance.pk}')
        except Exception as e:
            logger.error(f'Failed to create initial FSM state for Annotation {instance.pk}: {e}')


def handle_project_created(sender, instance, created, **kwargs):
    """
    Signal handler to automatically create initial state when a project is created.
    ...
    if created:
        try:
            ...
            StateManager.transition_state(
                entity=instance,
                new_state='CREATED',
                transition_name='create_project',
                reason='Project created automatically',
            )
            logger.info(f'Created initial FSM state for Project {instance.pk}')
        except Exception as e:
            logger.error(f'Failed to create initial FSM state for Project {instance.pk}: {e}')
