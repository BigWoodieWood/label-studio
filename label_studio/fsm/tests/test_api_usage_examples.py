...
class APIIntegrationExampleTests(TestCase):
    ...
    def test_json_schema_generation_for_api_docs(self):
        ...
        class APIAnnotationSubmissionTransition(BaseTransition):
            ...
            confidence_score: float = Field(
                ...
                ge=0.0, 
                le=1.0, 
                description="Annotator's confidence in the annotation (0.0-1.0)"
            )
            ...
            time_spent_seconds: int = Field(
                ...,
                ge=1,
                description="Time spent on annotation in seconds"
            )
            difficulty_level: str = Field(
                "medium",
                description="Perceived difficulty of the annotation task"
            )
            review_requested: bool = Field(
                False,
                description="Whether the annotator requests manual review"
            )
            tags: List[str] = Field(
    ...
    def test_webhook_integration_pattern(self):
        ...
        class WebhookTaskCompletionTransition(BaseTransition):
            ...
            webhook_responses: List[Dict[str, Any]] = Field(default_factory=list, description="Webhook response tracking")
    ...
    def test_api_versioning_and_backward_compatibility(self):
        ...
        context = TransitionContext(
            entity=self.mock_entity,
            current_state="CREATED",
            target_state=v1_transition.target_state
        )
