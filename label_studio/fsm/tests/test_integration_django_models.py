...
from fsm.models import TaskState, AnnotationState, get_state_model_for_entity
...
class DjangoModelIntegrationTests(TestCase):
    ...
    @patch('fsm.models.get_state_model_for_entity')
    ...
    def test_task_workflow_integration(self, mock_transition_state, mock_get_state_obj, mock_get_state_model):
        ...
        class CompleteTaskWithQualityTransition(BaseTransition):
            ...
            def transition(self, context: TransitionContext) -> Dict[str, Any]:
                ...
                return {
                    ...
                    "efficiency_ratio": (estimated_hours / self.actual_hours) if (estimated_hours and self.actual_hours) else None
                }
        
        # Execute the complete workflow
        
        # Step 1: Create task
        create_transition = CreateTaskTransition(
            created_by_id=100,
        ...
        assign_transition = AssignAndStartTaskTransition(
            assignee_id=200,  # Different from creator
            estimated_hours=4.5,
            priority="urgent"
        )
        ...
        complete_transition = CompleteTaskWithQualityTransition(
            quality_score=0.85,
            completion_notes="Task completed successfully with minor revisions",
            actual_hours=5.2
        )
    ...
    def test_annotation_review_workflow_integration(self):
        ...
        class ReviewAndApproveAnnotation(BaseTransition):
            ...
            def validate_transition(self, context: TransitionContext) -> bool:
                ...
                if self.reviewer_decision not in valid_decisions:
                    raise TransitionValidationError(
                        f"Invalid decision: {self.reviewer_decision}",
            ...
            def transition(self, context: TransitionContext) -> Dict[str, Any]:
                ...
                return {
                    ...
                    "confidence_vs_quality_diff": abs(submission_data.get("annotator_confidence", 0) - self.quality_score)
                }
        ...
        submit_transition = SubmitAnnotationForReview(
            ...
            annotation_time_seconds=300,  # 5 minutes
    ...
    def test_error_handling_with_django_models(self):
        ...
        valid_transition = AssignTaskWithConstraints(
            ...
            skill_requirements=["python", "labeling"]
        )
